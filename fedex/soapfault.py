...
#
# Generated Thu Dec 15 16:56:18 2011 by generateDS.py version 2.6a.
#
...
class Fault(GeneratedsSuper):
    ...
    def export(self, outfile, level, namespace_='soapenv:', name_='Fault', namespacedef_=''):
        showIndent(outfile, level)
        ...
        if self.hasContent_():
            ...
            outfile.write('/>\n')
    def exportAttributes(self, outfile, level, already_processed, namespace_='soapenv:', name_='Fault'):
        pass
    def exportChildren(self, outfile, level, namespace_='soapenv:', name_='Fault', fromsubclass_=False):
        if self.faultcode is not None:
            showIndent(outfile, level)
            outfile.write('<%sfaultcode>%s</%sfaultcode>\n' % (namespace_, self.gds_format_string(quote_xml(self.faultcode).encode(ExternalEncoding), input_name='faultcode'), namespace_))
        if self.faultstring is not None:
            showIndent(outfile, level)
            outfile.write('<%sfaultstring>%s</%sfaultstring>\n' % (namespace_, self.gds_format_string(quote_xml(self.faultstring).encode(ExternalEncoding), input_name='faultstring'), namespace_))
        if self.faultactor is not None:
            showIndent(outfile, level)
            outfile.write('<%sfaultactor>%s</%sfaultactor>\n' % (namespace_, self.gds_format_string(quote_xml(self.faultactor).encode(ExternalEncoding), input_name='faultactor'), namespace_))
    ...
    def buildChildren(self, child_, node, nodeName_, fromsubclass_=False):
        if nodeName_ == 'faultcode':
            faultcode_ = child_.text
            faultcode_ = self.gds_validate_string(faultcode_, node, 'faultcode')
            ...
            detail_ = etree_.tostring(child_)
            self.detail = detail_
# end class Fault
...
def parseLiteral(inFileName):
    ...
    # Enable Python to collect the space used by the DOM.
    doc = None
    sys.stdout.write('#from soapfault import *\n\n')
    sys.stdout.write('import soapfault as model_\n\n')
